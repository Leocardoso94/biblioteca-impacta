...
@Controller
public class DashBoardController extends WebMvcConfigurerAdapter {
	...
	public String homeAdmin(Model model, HttpSession session) throws ClassNotFoundException, SQLException {
		...
		model.addAttribute("contagemDeEmprestimos", Emprestimo.count());
		model.addAttribute("contagemDeReservas", Reserva.count());
		model.addAttribute("contagemDeExemplares", Exemplar.count());
		model.addAttribute("contagemDePessoas", Pessoa.count());
		return "admin/index";
	}
